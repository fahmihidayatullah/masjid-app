...
const Main = ({ mosqueName, onPrayerTime, runningText, setCurrentPage, iqomahTimes }) => {
    ...
    // Check for 45 minute after prayer times and show message
    useEffect(() => {
        ...
        Object.entries(prayerTimes).forEach(([prayerName, prayerTime]) => {
            // Calculate 1 hour after prayer time
            const [prayerHour, prayerMinute] = prayerTime.split(':').map(Number);
            const HourAfter = new Date();
            
            HourAfter.setHours(prayerHour, prayerMinute, 0, 0);
            HourAfter.setMinutes(HourAfter.getMinutes() + 45);
            const HourAfterStr = HourAfter.toTimeString().slice(0, 5);
            
            // Check if current time is 45 minutes after prayer and message hasn't been shown for this prayer today
            if (now === HourAfterStr && !shownMessagesForPrayers.includes(`${today}-${prayerName}`)) {
                setShow1PMMessage(true);
                setShownMessagesForPrayers(prev => [...prev, `${today}-${prayerName}`]);
                
                // Auto close after 30 seconds
                setTimeout(() => {
                    setShow1PMMessage(false);
                }, 30000);
            }
        });
        
        // Reset shown messages at midnight
            ...
            <div className="main-content-area">
                    ...
                    <div className="message-popup-overlay">
                        <div className="message-popup-content">
                            <button 
                                ...
                                onClick={() => setShow1PMMessage(false)}
                            >
                                ×
                            </button>
                            <div className="message-popup-text">
                                <p><strong>Assalamu'alaikum warahmatullahi wabarakatuh.</strong></p>
                                <p>Jamaah yang dirahmati Allah, setelah kita menyelesaikan ibadah di masjid ini, marilah kita kembali melanjutkan aktivitas dan tanggung jawab masing-masing. Sebagaimana firman Allah dalam QS. Al-Insyirah ayat 7–8:</p>
                                <p className="message-popup-quran">"Maka apabila engkau telah selesai dari suatu urusan, tetaplah bekerja keras untuk urusan yang lain, dan hanya kepada Tuhanmulah engkau berharap."</p>
                                <p>Semoga setiap aktivitas kita bernilai ibadah dan membawa keberkahan.</p>
                        ...
                        <div className="main-khatib-poster" style={{ 
                            ...
                            <div style={{ 
                                ...
                                <h1 style={{ 
                                    ...
                                    {isKhatibDateFriday() ? 'Khotbah Jum\'at' : 'Jadwal Kajian Berikutnya'}
                                </h1>
                ...
                <div className="main-prayer-times-grid-container">
                    ...
                    <div
                        ...
                        onClick={() => {
                            if (prayer !== 'now') {
                                onPrayerTime(prayer, 480); // Trigger iqomah with default 8 minutes
                            }
                        }}
                        style={{ cursor: prayer !== 'now' ? 'pointer' : 'default' }}
                    >
                        {prayer === 'now' ? (
                        <div>
